...
var intro = {
    ...
    // introduction text
    text:
        "Thank you for participating in our study!<br><br>In this study, you will be asked to <strong>read a short paragraph and answer a question</strong> about it. This shouldn't take longer than <strong>1 minute</strong>. If you get that right, you can <strong>continue the experiment to get a $6 bonus</strong>.<br>If you choose to continue, you'll see 19 articles that contain an image. Each image will have a description associated with it in order to make the image accessible to users who can't see it. Your task will be to rate these descriptions by answering questions about them. The whole HIT should take about <strong>25 minutes</strong>.<br><br>Please only participate once in this series of HITs.<br>Please do <strong>not</strong> participate on a mobile device since the page won't display properly.<br><small>If you have any questions or concerns, don't hesitate to contact me at ekreiss@stanford.edu</small>",
    legal_info:
...
var main = {
    ...
    render: function(CT) {
        ...
        let question_copy = {
            ...
            "no_irrelevant" : {
                "q": 'Does the description mention <strong>too much irrelevant</strong> information for accessibility needs?',
                "sl_left": 'Too much',
                "sl_right": 'Not too much',
            }
        }
        ...
        let img_file = exp.trial_info.main_trials[CT]['filename'];
        let img_path = 'stimuli/' + img_file;
        let context = exp.trial_info.main_trials[CT]['context'];
        let description = exp.trial_info.main_trials[CT]['description'];
        let topic = exp.trial_info.main_trials[CT]['topic'];
        let article_title = context;
        let article_text = context_paragraphs[context];
        ...
        $("#main").html(
            Mustache.render(viewTemplate, {
                ...
                q5: "For accessibility, how good is the description <strong>overall</strong>?",
