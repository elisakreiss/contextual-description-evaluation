...
# command to run: python clipscore.py cosid cosid_data/cosid_descriptions.json cosid_data/cosid_images/
# command to run: python clipscore.py cosid cosid_data/cosid_descriptions.json cosid_data/cosid_images/ --shuffle True
# python clipscore.py concadia-captions concadia_purpose_data/concadia_captions.json concadia_purpose_data/concadia_images/
# python clipscore.py concadia-descriptions concadia_purpose_data/concadia_descriptions.json concadia_purpose_data/concadia_images/
...
def parse_args():
    ...
    parser.add_argument(
        'data',
        type=str,
        help='data assessed by clipscore; options: cosid, concadia-captions, concadia-descriptions')
...
def get_clip_score(model, images, candidates, device, image_ids, w=2.5):
    '''
...
def main():
    ...
    if args.data == "cosid":
        results_filename = "cosid-clipscores"
    elif args.data == "concadia-captions":
        results_filename = "concadia-captions-clipscores"
    elif args.data == "concadia-descriptions":
        results_filename = "concadia-descriptions-clipscores"
    else:
        raise Exception("Please provide a valid dataset: cosid, concadia-captions, or concadia-descriptions")

    image_paths = [os.path.join(args.image_dir, path) for path in os.listdir(args.image_dir)
                   if path.endswith(('.png', '.jpg', '.JPG', '.jpeg', '.tiff'))]
    ...
    if args.shuffle: 
        ...
        results_filename = results_filename + "-shuffled"

    clipscores = []    
    candidate_control = []
    for index, img in enumerate(image_ids):
        image_feats = extract_all_images(
        [image_paths[index]], model, device, batch_size=64, num_workers=8)

        clipscore_value = get_clip_score(
        model, image_feats, [candidates[index]], device, img)

        clipscores.append(clipscore_value[0])
        candidate_control.append(candidates[index])
    
    df = pd.DataFrame({"imgstem_version": image_ids,
                       "texts": candidates,
                       "text_controls": candidate_control,
                       "clipscore": clipscores})
    # print(df)                       
    df.to_csv("results/" + results_filename + ".csv", index=False)
